...
export default function LoginPatient(){
    ...
    async function login(e){
        ...
        localStorage.setItem('patFname', res.user.firstName.toLowerCase());
        const tokenPat = res.token;
        console.log(tokenPat);
        localStorage.setItem('tokenPat', tokenPat);
        if(res.success == true){
            navigate(`/${res.user.userId}/PatientDashboard`);
        }
    }
        ...
        return (
            <div className="row">
                ...
                <div className='body-home' style={{marginTop:"20px"}}>
                    <p className="h4 text-center mb-4">Welcome to Ascle Patient Login</p>
                </div>
